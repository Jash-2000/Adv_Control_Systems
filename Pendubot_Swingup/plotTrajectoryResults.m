function plotTrajectoryResults(t, x, u, u_max)
    figure('Name', 'Part A: Trajectory Optimization', 'Position', [100 100 1200 700]);
    
    subplot(3,2,1);
    plot(t, rad2deg(x(1,:)), 'b-', 'LineWidth', 2);
    xlabel('Time (s)');
    ylabel('Angle (deg)');
    title('Joint 1 Angle (Shoulder - Actuated)');
    grid on;
    ...
    xlabel('Time (s)');
    ylabel('Angle (deg)');
    title('Joint 2 Angle (Elbow - Passive)');
    grid on;
    ...
    xlabel('Time (s)');
    ylabel('Torque (N-m)');
    title('Control Input (Shoulder Torque)');
