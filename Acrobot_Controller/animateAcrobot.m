function animateAcrobot(t, x, titleStr)
    % Physical parameters for animation (same as dynamics)
    L1 = 1.0;
    L2 = 1.0;
    ...
    for i = 1:3:length(t)
        ...
        % Compute link positions (base at origin)
        x1 = L1 * cos(theta1);
        y1 = L1 * sin(theta1);
        ...
        % Plot trajectory trace of end effector
        if i > 1
            for j = 1:i-1
                th1_j = x(1,j);
                th2_j = x(2,j);
